...
#define MSGDEF(x) \
    ...
    LPCTSTR CWindowUI::GetSuperClassName() const
    {
        return NULL;
    }
    ...
    HWND CWindowUI::CreateDuiWindow(HWND hwndParent, LPCTSTR pstrWindowName, DWORD dwStyle /*=0*/, DWORD dwExStyle /*=0*/)
    {
    ...
    HWND CWindowUI::Subclass(HWND hWnd)
    {
    ...
    void CWindowUI::Close(UINT nRet)
    {
        ASSERT(::IsWindow(m_hWnd));
        if (!::IsWindow(m_hWnd)) return;
    ...
    LRESULT CALLBACK CWindowUI::__WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
    {
        CWindowUI* pThis = NULL;
        if (uMsg == WM_NCCREATE) {
            LPCREATESTRUCT lpcs = reinterpret_cast<LPCREATESTRUCT>(lParam);
            pThis = static_cast<CWindowUI*>(lpcs->lpCreateParams);
            pThis->m_hWnd = hWnd;
            ::SetWindowLongPtr(hWnd, GWLP_USERDATA, reinterpret_cast<LPARAM>(pThis));
        } else {
            pThis = reinterpret_cast<CWindowUI*>(::GetWindowLongPtr(hWnd, GWLP_USERDATA));
    ...
    LRESULT CALLBACK CWindowUI::__ControlProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
    {
        CWindowUI* pThis = NULL;
        if (uMsg == WM_NCCREATE) {
            LPCREATESTRUCT lpcs = reinterpret_cast<LPCREATESTRUCT>(lParam);
            ...
            pThis = reinterpret_cast<CWindowUI*>(::GetProp(hWnd, _T("WndX")));
            if (uMsg == WM_NCDESTROY && pThis != NULL) {
                LRESULT lRes = ::CallWindowProc(pThis->m_OldWndProc, hWnd, uMsg, wParam, lParam);
                if (pThis->m_bSubclassed) pThis->Unsubclass();
    ...
    void CWindowUI::ResizeClient(int cx /*= -1*/, int cy /*= -1*/)
    {
        ASSERT(::IsWindow(m_hWnd));
        RECT rc = {0};
