...
namespace DUI
{
    ...
    void CComboBoxUI::PaintStatusImage(HDC hDC)
    {
        if (m_sArrowImage.IsEmpty())
            ...
            // make modify string
            CStringUI sModify = m_sArrowImage;
            ...
            CRectUI rcDest(0, 0, m_rcItem.right - m_rcItem.left, m_rcItem.bottom - m_rcItem.top);
            rcDest.Deflate(GetBorderSize(), GetBorderSize());
            rcDest.left = rcDest.right - m_nArrowWidth;

            CStringUI sSource = sModify.Mid(nPos1, nPos3 + 1 - nPos1);
            CStringUI sReplace;
