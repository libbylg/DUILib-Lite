...
namespace DUI
{
    class DUILIB_API CHotKeyWnd : public CWindowUI
    {
    public:
        CHotKeyWnd(void);
    ...
    public:
        ...
        CStringUI GetHotKeyName();
        void SetRules(WORD wInvalidComb, WORD wModifiers);
        CStringUI GetKeyName(UINT vk, BOOL fExtended);
    ...
    LRESULT CHotKeyWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
    {
        ...
        if (uMsg == WM_KILLFOCUS) lRes = OnKillFocus(uMsg, wParam, lParam, bHandled);
            ...
            CStringUI strText = GetHotKeyName();
            ::RECT rect;
    ...
    CStringUI CHotKeyWnd::GetKeyName(UINT vk, BOOL fExtended)
    {
        ...
        return CStringUI(szStr);
    }


    CStringUI CHotKeyWnd::GetHotKeyName()
    {
        ASSERT(::IsWindow(m_hWnd));

        CStringUI strKeyName;
        WORD wCode = 0;
        WORD wModifiers = 0;
    ...
    void CHotKeyUI::DoEvent(struct TEVENT_UI& event)
    {
    ...
    void CHotKeyUI::PaintText(HDC hDC)
    {
        ...
        CRenderUI::DrawText(hDC, m_pManager, rc, sText, dwTextColor, m_iFont, DT_SINGLELINE | m_uTextStyle);
    }
