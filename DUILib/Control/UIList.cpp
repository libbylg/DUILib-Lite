...
namespace DUI
{
    ...
    void CListUI::SetPos(RECT rc, BOOL bNeedInvalidate)
    {
        ...
        if (!m_pHeader->IsVisible()) {
            ...
            m_pHeader->SetPos(CRectUI(rc.left, 0, rc.right, 0), bNeedInvalidate);
        }
    ...
    void CListUI::DoEvent(struct TEVENT_UI& event)
    {
    ...
    {
        return UILIST_NORMAL;
    }

    TLISTINFO_UI* CListUI::GetListInfo()
    {
        return &m_ListInfo;
    }
    ...
    void CListUI::Scroll(int dx, int dy)
    {
        ...
        m_pList->SetScrollPos(CSizeUI(sz.cx + dx, sz.cy + dy));
    }
    ...
    void CListBodyUI::SetPos(RECT rc, BOOL bNeedInvalidate)
    {
        ...
        if (m_pOwner) {
            ...
            if (pHeader != NULL && pHeader->GetCount() > 0) {
                cxNeeded = MAX(0, pHeader->EstimateSize(CSizeUI(rc.right - rc.left, rc.bottom - rc.top)).cx);
                if (m_pHorizontalScrollBar && m_pHorizontalScrollBar->IsVisible()) {
    ...
    void CListBodyUI::DoEvent(struct TEVENT_UI& event)
    {
        if (!IsMouseEnabled() && event.Type > UIEVENT__MOUSEBEGIN && event.Type < UIEVENT__MOUSEEND) {
    ...
    {
        SetTextPadding(CRectUI(2, 0, 2, 0));
    ...
    void CListHeaderItemUI::DoEvent(struct TEVENT_UI& event)
    {
        if (!IsMouseEnabled() && event.Type > UIEVENT__MOUSEBEGIN && event.Type < UIEVENT__MOUSEEND) {
            if (m_pParent != NULL) m_pParent->DoEvent(event);
    ...
    {
        if (m_cxyFixed.cy == 0) return CSizeUI(m_cxyFixed.cx, m_pManager->GetDefaultFontInfo()->tm.tmHeight + 14);
    ...
    void CListHeaderItemUI::PaintText(HDC hDC)
    {
        ...
        int nLinks = 0;
        if (m_bShowHtml)
            CRenderUI::DrawHtmlText(hDC, m_pManager, rcText, sText, m_dwTextColor, \
                NULL, NULL, nLinks, m_iFont, m_uTextStyle);
        else
            CRenderUI::DrawText(hDC, m_pManager, rcText, sText, m_dwTextColor, \
    ...
    void CListElementUI::DoEvent(struct TEVENT_UI& event)
    {
        if (!IsMouseEnabled() && event.Type > UIEVENT__MOUSEBEGIN && event.Type < UIEVENT__MOUSEEND) {
            if (m_pOwner != NULL) m_pOwner->DoEvent(event);
    ...
    void CListElementUI::DrawItemBk(HDC hDC, const RECT & rcItem)
    {
        ...
        if (!IsEnabled()) {
            if (!pInfo->sDisabledImage.IsEmpty()) {
                if (!DrawImage(hDC, (LPCTSTR)pInfo->sDisabledImage)) {} else return;
            }
        }
        if (IsSelected()) {
            if (!pInfo->sSelectedImage.IsEmpty()) {
                if (!DrawImage(hDC, (LPCTSTR)pInfo->sSelectedImage)) {} else return;
            }
        }
        if ((m_uButtonState & UISTATE_HOT) != 0) {
            if (!pInfo->sHotImage.IsEmpty()) {
                if (!DrawImage(hDC, (LPCTSTR)pInfo->sHotImage)) {} else return;
            }
        }
        ...
        if (m_sBkImage.IsEmpty()) {
            if (!pInfo->sBkImage.IsEmpty()) {
                if (!DrawImage(hDC, (LPCTSTR)pInfo->sBkImage)) {} else return;
            }
        }
        ...
        if (pInfo->dwLineColor != 0) {
            if (pInfo->bShowRowLine) {
                ...
                CRenderUI::DrawLine(hDC, rcLine, 1, GetAdjustColor(pInfo->dwLineColor));
            }
    ...
    void CListLabelElementUI::DoEvent(struct TEVENT_UI& event)
    {
        if (!IsMouseEnabled() && event.Type > UIEVENT__MOUSEBEGIN && event.Type < UIEVENT__MOUSEEND) {
            if (m_pOwner != NULL) m_pOwner->DoEvent(event);
    ...
    {
        if (m_pOwner == NULL) return CSizeUI(0, 0);
        CStringUI sText = GetText();
        ...
        if (cXY.cx == 0 && m_pManager != NULL) {
            ...
            //if( pInfo->bShowHtml ) {
            //	int nLinks = 0;
            //	CRenderUI::DrawHtmlText(m_pManager->GetPaintDC(), m_pManager, rcText, sText, 0, NULL, NULL, nLinks, pInfo->nFont, DT_SINGLELINE | DT_CALCRECT | pInfo->uTextStyle & ~DT_RIGHT & ~DT_CENTER);
            //}
            //else {
            //	CRenderUI::DrawText(m_pManager->GetPaintDC(), m_pManager, rcText, sText, 0, pInfo->nFont, DT_SINGLELINE | DT_CALCRECT | pInfo->uTextStyle & ~DT_RIGHT & ~DT_CENTER);
            //}
    ...
    CListTextElementUI::~CListTextElementUI()
    {
        ...
        for (int it = 0; it < m_aTexts.GetSize(); it++) {
            pText = static_cast<CStringUI*>(m_aTexts[it]);
    ...
    {
        CStringUI* pText = static_cast<CStringUI*>(m_aTexts.GetAt(iIndex));
        if (pText) {
            if (!IsResourceText())
                return pText->GetData();
            return CResourceUI::GetInstance()->GetText(*pText);
        }
        return NULL;
    }
    ...
    void CListTextElementUI::SetText(int iIndex, LPCTSTR pstrText)
    {
        ...
        CStringUI* pText = static_cast<CStringUI*>(m_aTexts[iIndex]);
        if ((pText == NULL && pstrText == NULL) || (pText && *pText == pstrText)) return;

        if (pText) { delete pText; pText = NULL; }
        m_aTexts.SetAt(iIndex, new CStringUI(pstrText));
    ...
    CStringUI* CListTextElementUI::GetLinkContent(int iIndex)
    {
        if (iIndex >= 0 && iIndex < m_nLinks) return &m_sLinks[iIndex];
        return NULL;
    }

    void CListTextElementUI::DoEvent(struct TEVENT_UI& event)
    {
        if (!IsMouseEnabled() && event.Type > UIEVENT__MOUSEBEGIN && event.Type < UIEVENT__MOUSEEND) {
            if (m_pOwner != NULL) m_pOwner->DoEvent(event);
    ...
    void CListContainerElementUI::DoEvent(struct TEVENT_UI& event)
    {
        if (!IsMouseEnabled() && event.Type > UIEVENT__MOUSEBEGIN && event.Type < UIEVENT__MOUSEEND) {
            if (m_pOwner != NULL) m_pOwner->DoEvent(event);
            else CContainerUI::DoEvent(event);
    ...
    void CListContainerElementUI::SetPos(RECT rc, BOOL bNeedInvalidate)
    {
        ...
        const CListHeaderUI* pHeader = pList->GetHeader();
        ...
        for (int i = 0; i < nCount; i++) {
            ...
            const CControlUI* pHeaderItem = pHeader->GetItemAt(i);
            if (pHeaderItem == NULL) {
                return;
            }
            const RECT rcHeaderItem = pHeaderItem->GetPos();
