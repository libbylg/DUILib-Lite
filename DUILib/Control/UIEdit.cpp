...
namespace DUI
{
    ...
    LRESULT CEditWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
    {
        ...
        if (uMsg == WM_CREATE) {
            ...
            if (clrColor < 0xFF000000) {
                ...
                HBITMAP hBmpEditBk = CRenderUI::GenerateBitmap(m_pOwner->GetManager(), rcWnd, m_pOwner, clrColor);
    ...
    void CEditUI::DoEvent(struct TEVENT_UI& event)
    {
...
#pragma warning(default:4302)
   ...
   //         }
    ...
    LPCTSTR CEditUI::GetTipValue()
    {
        ...
        return CResourceUI::GetInstance()->GetText(m_sTipValue);
    }
    ...
    {
        if (m_cxyFixed.cy == 0) return CSizeUI(m_cxyFixed.cx, m_pManager->GetFontInfo(GetFont())->tm.tmHeight + 6);
    ...
    void CEditUI::PaintText(HDC hDC)
    {
        ...
        CStringUI sDrawText = GetText();
        CStringUI sTipValue = GetTipValue();
        if (sDrawText == sTipValue || sDrawText == _T("")) {
            mCurTextColor = m_dwTipValueColor;
            sDrawText = sTipValue;
        } else {
            CStringUI sTemp = sDrawText;
            if (m_bPasswordMode) {
                sDrawText.Empty();
        ...
        if (IsEnabled()) {
            CRenderUI::DrawText(hDC, m_pManager, rc, sDrawText, mCurTextColor, \
                m_iFont, DT_SINGLELINE | m_uTextStyle);
        } else {
            CRenderUI::DrawText(hDC, m_pManager, rc, sDrawText, m_dwDisabledTextColor, \
                m_iFont, DT_SINGLELINE | m_uTextStyle);
        }
    }
}
