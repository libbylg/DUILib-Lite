...
namespace DUI
{
    ...
    class CComboWnd : public CWindowUI, public INotifyUI
    {
    public:
        ...
        void Notify(struct TNOTIFY_UI& msg) override;
    ...
    void CComboWnd::Notify(struct TNOTIFY_UI& msg)
    {
    ...
    TLISTINFO_UI* CComboUI::GetListInfo()
    {
        return &m_ListInfo;
    }
    ...
    void CComboUI::DoEvent(struct TEVENT_UI& event)
    {
    ...
    {
        if (m_cxyFixed.cy == 0) return CSizeUI(m_cxyFixed.cx, m_pManager->GetDefaultFontInfo()->tm.tmHeight + 12);
    ...
    void CComboUI::PaintStatusImage(HDC hDC)
    {
        ...
        if (!m_sNormalImage.IsEmpty()) {
            if (!DrawImage(hDC, (LPCTSTR)m_sNormalImage)) {} else return;
        }
    }
    ...
    void CComboUI::PaintText(HDC hDC)
    {
        ...
        if (IsEnabled()) {
            if (m_bShowHtml)
                CRenderUI::DrawHtmlText(hDC, m_pManager, rc, sText, m_dwTextColor, NULL, NULL, nLinks, m_iFont, m_uTextStyle);
            else
                CRenderUI::DrawText(hDC, m_pManager, rc, sText, m_dwTextColor, m_iFont, m_uTextStyle);
        } else {
            if (m_bShowHtml)
                CRenderUI::DrawHtmlText(hDC, m_pManager, rc, sText, m_dwDisabledTextColor, NULL, NULL, nLinks, m_iFont, m_uTextStyle);
            else
                CRenderUI::DrawText(hDC, m_pManager, rc, sText, m_dwDisabledTextColor, m_iFont, m_uTextStyle);
        }
    }
