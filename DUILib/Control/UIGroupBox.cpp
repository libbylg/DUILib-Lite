...
namespace DUI
{
    UI_IMPLEMENT_CONTROL(CGroupBoxUI)
    ...
    void CGroupBoxUI::PaintText(HDC hDC)
    {
        ...
        CRenderUI::DrawText(hDC, m_pManager, rcText, sText, dwTextColor, m_iFont, m_uTextStyle, GetAdjustColor(m_dwBackColor));
    }
        ...
        if (nBorderSize > 0) {
            ...
            if (cxyBorderRound.cx > 0 || cxyBorderRound.cy > 0)//»­Ô²½Ç±ß¿ò
            {
                if (IsFocused() && m_dwFocusBorderColor != 0)
                    CRenderUI::DrawRoundRect(hDC, rcItem, nBorderSize, cxyBorderRound.cx, cxyBorderRound.cy, GetAdjustColor(m_dwFocusBorderColor));
                else
                    CRenderUI::DrawRoundRect(hDC, rcItem, nBorderSize, cxyBorderRound.cx, cxyBorderRound.cy, GetAdjustColor(m_dwBorderColor));
            } else {
                if (IsFocused() && m_dwFocusBorderColor != 0)
                    CRenderUI::DrawRect(hDC, rcItem, nBorderSize, GetAdjustColor(m_dwFocusBorderColor));
                else
                    CRenderUI::DrawRect(hDC, rcItem, nBorderSize, GetAdjustColor(m_dwBorderColor));
            }
        }
