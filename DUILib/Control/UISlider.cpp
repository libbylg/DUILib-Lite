...
namespace DUI
{
            ...
            if (IsEnabled()) {
                ...
                if (m_bHorizontal) {
                    if (event.ptMouse.x >= m_rcItem.right - m_szThumb.cx / 2) {
                        ...
                        nValue = m_nMin + 1.0f * (m_nMax - m_nMin) * (event.ptMouse.x - m_rcItem.left - m_szThumb.cx / 2) / (m_rcItem.right - m_rcItem.left - m_szThumb.cx);
                    }
                } else {
                    if (event.ptMouse.y >= m_rcItem.bottom - m_szThumb.cy / 2) {
                        ...
                        nValue = m_nMin + 1.0f * (m_nMax - m_nMin) * (m_rcItem.bottom - event.ptMouse.y - m_szThumb.cy / 2) / (m_rcItem.bottom - m_rcItem.top - m_szThumb.cy);
                    }
                }
    ...
    void CSliderUI::PaintForeImage(HDC hDC)
    {
        ...
        if (!m_sThumbImage.IsEmpty()) {
            m_sImageModify.Empty();
            m_sImageModify.SmallFormat(_T("dest='%d,%d,%d,%d'"), rcThumb.left, rcThumb.top, rcThumb.right, rcThumb.bottom);
            if (!DrawImage(hDC, (LPCTSTR)m_sThumbImage, (LPCTSTR)m_sImageModify)) {} else return;
        }
    }
}
