...
namespace DUI
{
    ...
    void COptionUI::PaintStatusImage(HDC hDC)
    {
        if (IsSelected()) {
            if (!m_sSelectedStateImage.IsEmpty() && m_nSelectedStateCount > 0) {
                TDRAWINFO_UI info;
                info.Parse(m_sSelectedStateImage, _T(""), m_pManager->GetDPIObj());
    ...
    void COptionUI::PaintForeImage(HDC hDC)
    {
        if (IsSelected()) {
            if (!m_sSelectedForeImage.IsEmpty()) {
                if (!DrawImage(hDC, (LPCTSTR)m_sSelectedForeImage)) {} else return;
            }
        }
    ...
    void COptionUI::PaintText(HDC hDC)
    {
        if ((m_uButtonState & UISTATE_SELECTED) != 0) {
            ...
            if (m_bShowHtml)
                CRenderUI::DrawHtmlText(hDC, m_pManager, rc, sText, IsEnabled()?m_dwTextColor:m_dwDisabledTextColor, \
                    NULL, NULL, nLinks, iFont, m_uTextStyle);
            else
                CRenderUI::DrawText(hDC, m_pManager, rc, sText, IsEnabled()?m_dwTextColor:m_dwDisabledTextColor, \
                    iFont, m_uTextStyle);
